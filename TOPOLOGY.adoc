...
|===
|Type |Module |Purpose
|`+Distribution+` |Types.idr |Sum type: Normal, Uniform, Beta,
Bernoulli, Custom

|`+TernaryBool+` |Types.idr |True / False / Unknown with Kleene logic

|`+ProbabilityValue+` |Types.idr |Value in [0,1] with dependent type
proof

|`+ConfidenceInterval+` |Types.idr |Lower/upper bounds at confidence
level
